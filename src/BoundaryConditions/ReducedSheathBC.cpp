    ...
    [[maybe_unused]] const NekDouble &time)
{
    ...
    Array<OneD, NekDouble> &ee_bc = m_bndExp[ee_idx]->UpdatePhys();
    for (int p = 0; p < m_nEdgePts; ++p)
    {
        NekDouble Te = (2.0 / 3.0) * Fwd[ee_idx][p] / ne[p];

        NekDouble ve =
            -std::sqrt(Te / (me * 2 * M_PI)) * (1.0 - Ge) * std::exp(wall / Te);

        ee_bc[p] = (2.0 / 3.0) * gamma_e * Fwd[ee_idx][p] * ve * bn[p];
    }

    // Electron Pressure
    m_bndExp[ee_idx]->IProductWRTBase(ee_bc, m_bndExp[ee_idx]->UpdateCoeffs());
}
